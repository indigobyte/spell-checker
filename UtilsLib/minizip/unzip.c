...
extern int ZEXPORT unzGetFileList (unzFile file, void (*pfCallback)(const char *szFileName, void *pParam), void *pParam)
{
    unz_s* s;
    int err;
    ...
    while (err == UNZ_OK)
    {
        ...
        if (err == UNZ_OK)
        {
			pfCallback(szCurrentFileName, pParam);
            err = unzGoToNextFile(file);
        }
    }
    ...
    s->cur_file_info = cur_file_infoSaved;
    s->cur_file_info_internal = cur_file_info_internalSaved;
	return (err == UNZ_END_OF_LIST_OF_FILE) ? UNZ_OK : err;
}
